...
//     // Extrait les <p> et les pagine (4 paragraphes par page par défaut)
...
function OpenBook({ book, onFoundHint }) {
  ...
  // Déclenche l'indice quand le bon livre est ouvert
  useEffect(() => {
    if (book === 'bleu' && typeof onFoundHint === 'function') {
      onFoundHint();
    }
...
// export default OfficeGameEmbedEmbed;

// --- FlipBook (pagination + animation simple) ---
function FlipBook({ title, pages, coverColor = '#f0eadd' }) {
  const [pageIndex, setPageIndex] = useState(0);
  const [isFlipping, setIsFlipping] = useState(false);
  const [flipDir, setFlipDir] = useState('next'); // 'next' | 'prev'
